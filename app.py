...
st.markdown(
    ...
    <div class="docsift-fixed-header">
        <h1>DocSift</h1>
        <p>Turn long work documents into clear summaries, action items, and risk notes.</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
def render_pro_upsell(feature_name: str):
    ...
    st.info(
        ...
        "Upgrade to DocSift Pro to unlock all professional workflow tools:\n"
        "- Key Points\n"
        "- Action Items\n"
        "- Risks & Red Flags\n"
        "- Rewrite for Clarity\n"
...
def summarize_text(text: str) -> str:
    ...
    system_message = (
        "You create executive summaries for busy professionals. "
        "Write a clear, structured summary in short paragraphs. "
        "Include purpose/context, key findings, decisions (if any), and implications/next steps if present. "
...
def build_full_report(text: str) -> str:
    ...
    sections = [
        ...
        "## 4. Risks & Red Flags",
        risks,
        "## 5. Simplify the Jargon (Plain Language)",
        eli12,
        "## 6. Training / Onboarding Guide",
        study_guide,
    ]
...
with st.sidebar:
    ...
    2. Confirm you see **“Document loaded successfully.”**  
    3. Use the tabs to generate what you need:
    - **Executive Summary** (quick brief)
    - **Key Points** and **Action Items**
    - **Risks & Red Flags** (gaps, vague language, missing info)
    - **Rewrite for Clarity** (shareable version)
    - **Simplify the Jargon** (plain-language explanation)

    DocSift does not store your documents. Everything runs during your session.
    """)
...
if uploaded_file is None:
    ...
    st.write(
        ...
        "DocSift will extract the text and help you generate an executive summary, "
        "key points, action items, risks/red flags, and shareable rewrites."
    )
    ...
    if error_message:
        ...
        tab_summary, tab_keypoints, tab_actions, tab_risks, tab_rewrite, tab_plain, tab_training, tab_full, tab_overview = st.tabs([
        "Executive Summary",
        "Key Points",
        "Action Items",
        "Risks & Red Flags",
        ...
        with tab_overview:
            st.subheader("Document Text (extracted)")
            st.write("Review the extracted text here before generating a brief or analysis.")
        ...
        with tab_summary:
            ...
            st.write(
                "Generate a clear, professional brief you can paste into an email, ticket, or meeting notes. "
                "Focus is on what matters: purpose, key findings, decisions, and implications."
            )
        ...
        with tab_risks:
            ...
            if not st.session_state.is_pro_user:
                ...
                st.write(
                    ...
                    "Ideal for policies, vendor docs, contracts, and internal proposals."
                )
                ...
                # ---- Plain tab ----
        with tab_plain:
            st.subheader("Simplify the Jargon (plain-language explanation)")

            if not st.session_state.is_pro_user:
                render_pro_upsell("Simplify the Jargon")
            else:
                st.write(
                    "Turn complex language into a plain-English explanation that’s easy to share with stakeholders "
                    "or teammates who aren’t deep in the details."
                )

                if st.button("Generate Plain-Language Version"):
                    try:
        ...
        with tab_training:
            st.subheader("Training / Onboarding Guide")

            if not st.session_state.is_pro_user:
                render_pro_upsell("Training / Onboarding Guide")
            else:
                st.write(
                    "Turn the document into a structured guide for training and onboarding. "
                    "Includes key concepts and Q&A so teams can learn the material quickly."
                )
                ...
                if st.button("Generate Training Guide"):
                    try:
                        ...
                        if isinstance(study_guide, str) and study_guide.startswith("[Error"):
                            st.error(
                                "There was a problem generating the guide. "
                                "Please try again in a moment."
                            )
                ...
                # ---- Full Report tab ----
        with tab_full:
            st.subheader("Full Brief (All Sections)")

            if not st.session_state.is_pro_user:
                render_pro_upsell("Full Brief")

            
            else:
                st.write(
                    "Generate a complete brief: executive summary, key points, action items, risks/red flags, "
                    "plain-language explanation, clarity rewrite, and a training/onboarding guide."
                )

                if st.button("Generate Full Brief"):
                    with st.spinner(
                        "Generating full brief (summary, key points, actions, risks, plain language, "
                        "clarity rewrite, guide)..."
                    ):
